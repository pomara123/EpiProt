...
public class View extends JFrame implements Presenter.View, ActionListener {
	...
	public View() {
		// setUIFont (new
		// javax.swing.plaf.FontUIResource("monospaced",Font.PLAIN,10));
		...
		try {
			...
			// UIManager.put("TextPane.font", new Font("Currier",
			// Font.PLAIN,10));
		} catch (Exception exc) {
	...
	protected void initKeyMap(final JEditorPane edit) {
		...
		kMap.addActionForKeyStroke(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, InputEvent.SHIFT_MASK), a);
	}
	...
	public void insertLineMiddle(String header, String line) {
		...
		//System.out.println("headerpane length" + headerPane.getDocument().getLength());
		//System.out.println("editorPane length" + editorPane.getDocument().getLength());
		insertLine(proteinAcc, header, line, headerPane.getDocument().getLength(),
	...
	private String getHTML(String tag, String id, String header, String text) {
		String html = "<" + tag + " style=\"LINE-HEIGHT:0px;\" id=\"" + id + "\" name=\"" + header + "\">" + text + "</"
				+ tag + ">";
	...
	public void loadDocument() {
		...
		try {
			...
			insertNewLine(11, headerDocument, headerPaneKit);

			for (Header header : headers) {
				headerPaneKit.insertHTML(headerDocument, headerDocument.getLength(),
						getHTML("pre", header.id, header.name, header.name), 0, 0, null);
			}
